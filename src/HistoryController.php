...
class HistoryController {
	...
	public function handle_record() {
		try {
			...
			// Trim to history limit. Lite is capped at 10; Pro uses the configured limit.
			...
			error_log( 'WPQP: HistoryController handle_record: ' . $e->getMessage() );
	...
	public function handle_get() {
		try {
			...
			// Enforce limit: Lite is capped at 10; Pro uses the configured limit.
			if ( function_exists( 'wpqp_is_pro' ) && wpqp_is_pro() ) {
				$options       = Options::get_all();
				$history_limit = isset( $options['pro']['history_limit'] ) ? (int) $options['pro']['history_limit'] : 50;
				$history_limit = max( 10, min( 200, $history_limit ) );
			...
			error_log( 'WPQP: HistoryController handle_get: ' . $e->getMessage() );
			wp_send_json_error( array( 'message' => __( 'An unexpected error occurred.', 'wp-quick-palette' ) ), 500 );
		}
	}
	...
	public function handle_clear() {
		try {
			...
			error_log( 'WPQP: HistoryController handle_clear: ' . $e->getMessage() );
			wp_send_json_error( array( 'message' => __( 'An unexpected error occurred.', 'wp-quick-palette' ) ), 500 );
		}
	}
	...
	public function track_edit_page_visit() {
		try {
			...
			error_log( 'WPQP: HistoryController track_edit_page_visit: ' . $e->getMessage() );
			return;
		}
	}
}
