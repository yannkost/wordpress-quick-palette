...
class FavoritesController {
	...
	public function handle_toggle() {
		try {
			...
			error_log( 'WPQP: FavoritesController handle_toggle: ' . $e->getMessage() );
	...
	public function handle_get() {
		try {
			...
			error_log( 'WPQP: FavoritesController handle_get: ' . $e->getMessage() );
			wp_send_json_error( array( 'message' => __( 'An unexpected error occurred.', 'wp-quick-palette' ) ), 500 );
		}
	}
	...
	public function handle_reorder() {
		try {
			...
			// Unknown items (not in $current_favorites) are silently dropped.
			$new_favorites = array();
			...
			error_log( 'WPQP: FavoritesController handle_reorder: ' . $e->getMessage() );
			wp_send_json_error( array( 'message' => __( 'An unexpected error occurred.', 'wp-quick-palette' ) ), 500 );
		}
	}
}
